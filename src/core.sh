...
info() {
    ...
    vtcp)
        ...
        is_url="vless://$uuid@$is_addr:$port?encryption=none&type=tcp#singbox-vless-tcp-$is_addr"
        ;;
    ws | tcp | h2 | quic | http*)
        if [[ $host ]]; then
            is_color=45
    ...
    anytls)
        ...
        if [[ $is_anytls_domain ]]; then
            ...
            is_url="anytls://$password@$is_anytls_domain:$port#singbox-$net-$is_anytls_domain"
        else
            is_insecure=1
            is_info_show=(0 1 2 10 8 20)
            is_info_str=($is_protocol $is_addr $port $password tls true)
            is_url="anytls://$password@$is_addr:$port?allowInsecure=1#singbox-$net-$is_addr"
        fi
        ;;
    direct)
